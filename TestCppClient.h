...
#define MAX_INSTRUMENTS_NUM 128
...
public:
	...
	bool setCommand();
	bool clearCommand();
	void addInstrument(Contract instrument);
	int getDepth(char* name, char* value);
	int getKlines(char* name, int duration,char* value);
	int getBalance(char* info);
	int placeOrder(char *name, bool isBuy, double quantity, char *result);
	int getPositions(char *result);
	int getInstruments(char *result);
...
private:
	...
	Contract m_instruments[MAX_INSTRUMENTS_NUM];
	int m_numOfInstruments;

	DepthItem m_depthList[MAX_INSTRUMENTS_NUM];
	...
	StructResponse m_response;
	ContractInfo m_contractInfo[MAX_INSTRUMENTS_NUM];
	int m_numOfContractsInfo;
};
