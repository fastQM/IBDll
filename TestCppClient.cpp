...
#define MAX_TIMEOUT 5000*2
#define TICKER_START_ID 1000
#define DEPTH_START_ID 2000
#define CONTRACT_START_ID 200
#define ACCOUNT_START_ID 9000
...
TestCppClient::TestCppClient() :
    ...
    , m_pReader(0)
    , m_extraAuth(false)
	, m_commandStatus(CS_NONE)
{
	// 不可清0
	//memset(m_instruments, 0, sizeof(Contract) * MAX_INSTRUMENTS_NUM);
	m_numOfInstruments = 0;

	memset(m_depthList, 0, sizeof(DepthItem) * MAX_INSTRUMENTS_NUM);
	memset(&m_response, 0, sizeof(StructResponse));
	
	memset(m_contractInfo, 0, sizeof(ContractInfo) * MAX_INSTRUMENTS_NUM);
	m_numOfContractsInfo = 0;
...
bool TestCppClient::setCommand() {
	if (m_state != ST_COMMAND) {
		printf("初始化未完成，请稍后再试");
		return false;
	}
	if (m_commandStatus != CS_NONE) {
		printf("命令执行中或者错误，请稍后再试");
		return false;
	}
	m_commandStatus = CS_OPEN;
	return true;

}

bool TestCppClient::clearCommand() {
	m_commandStatus = CS_NONE;
	return true;
}

void TestCppClient::addInstrument(Contract instrument) {
	m_instruments[m_numOfInstruments].conId = instrument.conId;
	m_instruments[m_numOfInstruments].symbol = instrument.symbol;
	m_instruments[m_numOfInstruments].currency = instrument.currency;
	m_instruments[m_numOfInstruments].exchange = instrument.exchange;
	m_instruments[m_numOfInstruments].lastTradeDateOrContractMonth = instrument.lastTradeDateOrContractMonth;
	m_instruments[m_numOfInstruments].secType = instrument.secType;
	m_numOfInstruments++;
}

int TestCppClient::getDepth(char* name, char* value) {
	AcquireSRWLockShared(&mSrwlockDepth);

	for (int i = 0; i < m_numOfInstruments; i++) {
		if (strcmp(name, m_instruments[i].symbol.c_str()) == 0) {
			Document d;
			d.SetObject();
			d.AddMember(rapidjson::StringRef("name"), rapidjson::StringRef(name), d.GetAllocator());
			//d.AddMember(rapidjson::StringRef("tradingDate"), rapidjson::StringRef(m_depthList[i].tradingDate), d.GetAllocator());
			d.AddMember(rapidjson::StringRef("updateTime"), rapidjson::StringRef(m_depthList[i].time), d.GetAllocator());
			d.AddMember(rapidjson::StringRef("bid"), m_depthList[i].bidPrice, d.GetAllocator());
			d.AddMember(rapidjson::StringRef("bidvolumn"), m_depthList[i].bidSize, d.GetAllocator());
			d.AddMember(rapidjson::StringRef("ask"), m_depthList[i].askPrice, d.GetAllocator());
			d.AddMember(rapidjson::StringRef("askvolumn"), m_depthList[i].askSize, d.GetAllocator());
			//d.AddMember(rapidjson::StringRef("lastPrice"), m_depthList[i].lastPrice, d.GetAllocator());
			//d.AddMember(rapidjson::StringRef("closePrice"), m_depthList[i].closePrice, d.GetAllocator());
...
void TestCppClient::tickDataOperation()
{
	...
	for (int i = 0; i < m_numOfInstruments; i++) {
		m_pClient->reqMktData(TICKER_START_ID+i, m_instruments[i], "", false, TagValueListSPtr());
	}
    //! [reqmktdata]
...
void TestCppClient::marketDepthOperations()
{
	...
	//! [reqmarketdepth]
	//m_pClient->reqMktDepth(2001, ContractSamples::ContractA50(), 5, TagValueListSPtr());
	//! [reqmarketdepth]
	//sleep(2);
...
int TestCppClient::placeOrder(char* name, bool isBuy, double quantity, char *result) {

	Contract contract;
	bool isFound = false;
	for (int i = 0; i < m_numOfInstruments; i++) {
		if (strcmp(m_instruments[i].symbol.c_str(), name) == 0) {
			contract = m_instruments[i];
			isFound = true;
			break;
		}
	}
	...
	if (isFound) 
	{
		...
		m_pClient->placeOrder(m_response.reqId, contract, OrderSamples::MarketOrder(action, quantity));

		printf("等待接收数据...\n");

		DWORD dw = WaitForSingleObject(m_semaphore, MAX_TIMEOUT);
		if (dw == WAIT_TIMEOUT || dw == WAIT_FAILED)
		{
			printf("下单错误或者等待超时，取消所有交易!\n");
			m_pClient->reqGlobalCancel();
			doc.AddMember(rapidjson::StringRef("status"), rapidjson::StringRef("cancel"), doc.GetAllocator());
		}
...
int TestCppClient::getBalance(char* info) {
	...
	counter++;
	m_response.reqId = 9000+counter;
	m_response.doc = &doc;

	m_semaphore = CreateSemaphore(NULL, 0, 1, NULL);

	m_pClient->reqAccountSummary(m_response.reqId, "All", AccountSummaryTags::getAllTags());
	...
	printf("收到数据...\n");
	m_pClient->cancelAccountSummary(m_response.reqId);
	CloseHandle(m_semaphore);
	m_semaphore = NULL;
...
int TestCppClient::getKlines(char* name, int duration, char* value)
{
	static int counter = 0;
	...
	if (isFound) {
		...
		//std::strftime(queryTime, 200, "%Y%m%d 23:59:59", timeinfo);


		char buff[128];
		memset(buff, 0, 128);
		if (duration < 365) {
			sprintf(buff, "%d D", duration);
		}
		else {
			int years = duration / 365 + 1;
			sprintf(buff, "%d Y", years);
		}

		printf("Duration:%s\n", buff);

		m_klines = (KlineItem*)malloc(sizeof(KlineItem)*duration);
		if (m_klines == NULL) {
			printf("分配内存失败\n");
			return false;
		}
		...
		m_pClient->reqHistoricalData(tickerID, contract, "", buff, "1 day", "TRADES", 0, 1, TagValueListSPtr());
		printf("等待接收数据...\n");

		DWORD dw = WaitForSingleObject(m_semaphore, MAX_TIMEOUT*12);	// 1 minute
		if (dw == WAIT_TIMEOUT||dw== WAIT_FAILED)
		{
		...
		Document objArray;
		objArray.SetArray();
		for (int i = 0; i < m_klinesCounter; i++) {
			Document d;
			d.SetObject();
			d.AddMember(rapidjson::StringRef("open"), m_klines[i].open, objArray.GetAllocator());
			//d.AddMember(rapidjson::StringRef("tradingDate"), rapidjson::StringRef(m_depthList[i].tradingDate), d.GetAllocator());
			d.AddMember(rapidjson::StringRef("high"), m_klines[i].high, objArray.GetAllocator());
			d.AddMember(rapidjson::StringRef("low"), m_klines[i].low, objArray.GetAllocator());
			d.AddMember(rapidjson::StringRef("close"), m_klines[i].close, objArray.GetAllocator());
			d.AddMember(rapidjson::StringRef("volume"), m_klines[i].volume, objArray.GetAllocator());
			d.AddMember(rapidjson::StringRef("date"), rapidjson::StringRef(m_klines[i].date), objArray.GetAllocator());
			objArray.PushBack(d, objArray.GetAllocator());
		}

		StringBuffer buffer;
		Writer<StringBuffer> writer(buffer);
		objArray.Accept(writer);
...
int TestCppClient::getInstruments(char *result) 
{
	Document objArray;
	objArray.SetArray();
	for (int i = 0; i < m_numOfContractsInfo; i++) {
		Document d;
		d.SetObject();
		d.AddMember(rapidjson::StringRef("symbol"), rapidjson::StringRef(m_contractInfo[i].symbol), objArray.GetAllocator());
		d.AddMember(rapidjson::StringRef("conId"), m_contractInfo[i].conId, objArray.GetAllocator());
		d.AddMember(rapidjson::StringRef("multiple"), rapidjson::StringRef(m_contractInfo[i].multiple), objArray.GetAllocator());
		d.AddMember(rapidjson::StringRef("timezone"), rapidjson::StringRef(m_contractInfo[i].timezone), objArray.GetAllocator());
		d.AddMember(rapidjson::StringRef("trading"), rapidjson::StringRef(m_contractInfo[i].trading), objArray.GetAllocator());
		objArray.PushBack(d, objArray.GetAllocator());
	}
...
int TestCppClient::getPositions(char *result)
{
	Document doc;
	doc.SetObject();

	m_response.reqId = 8888;
	m_response.doc = &doc;
...
void TestCppClient::historicalDataRequests()
{
	...
	std::time(&rawtime);
    timeinfo = std::localtime(&rawtime);
	std::strftime(queryTime, 200, "%Y%m%d %H:%M:%S", timeinfo);

	m_pClient->reqHistoricalData(4001, ContractSamples::EurGbpFx(), queryTime, "200 D", "1 day", "MIDPOINT", 1, 1, TagValueListSPtr());
...
{
	for (int i = 0; i < m_numOfInstruments; i++) {
		m_pClient->reqContractDetails(CONTRACT_START_ID + i, m_instruments[i]);
	}
...
// result sample
/*
	Account List: U9856812
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: AccountType, Value: INDIVIDUAL, Currency:
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: Cushion, Value: 1, Currency:
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: DayTradesRemaining, Value: 3, Currency:
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: LookAheadNextChange, Value: 0, Currency:
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: AccruedCash, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: AvailableFunds, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: BuyingPower, Value: 62121.84, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: EquityWithLoanValue, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: ExcessLiquidity, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: FullAvailableFunds, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: FullExcessLiquidity, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: FullInitMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: FullMaintMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: GrossPositionValue, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: InitMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: LookAheadAvailableFunds, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: LookAheadExcessLiquidity, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: LookAheadInitMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: LookAheadMaintMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: MaintMarginReq, Value: 0.00, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: NetLiquidation, Value: 15530.46, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: SMA, Value: 13801.92, Currency: USD
	Acct Summary. ReqId: 9001, Account: U9856812, Tag: TotalCashValue, Value: 15530.46, Currency: USD
...
void TestCppClient::tickPrice( TickerId tickerId, TickType field, double price, int canAutoExecute) {
	...
	int index = tickerId - TICKER_START_ID;
	SYSTEMTIME systm;
	GetLocalTime(&systm);
	char now[128];
	memset(now, 0, 128);
	sprintf(now, "%04d-%02d-%02d %02d:%02d:%02d", systm.wYear, systm.wMonth, systm.wDay, systm.wHour, systm.wMinute, systm.wSecond);
	//AcquireSRWLockExclusive(&mSrwlockDepth);
	switch (field) {
	case BID:
		m_depthList[index].bidPrice = price;
		strcpy(m_depthList[index].time, now);
		break;
	case ASK:
		m_depthList[index].askPrice = price;
		strcpy(m_depthList[index].time, now);
		break;
	default:
		//printf("Error: TickType:%d\n", field);
		break;
	}
...
void TestCppClient::tickSize( TickerId tickerId, TickType field, int size) {
	...
	switch (field) {
	case BID_SIZE:
		m_depthList[index].bidSize = size;
		strcpy(m_depthList[index].time, now);
		break;
	case ASK_SIZE:
		m_depthList[index].askSize = size;
		strcpy(m_depthList[index].time, now);
		break;
	default:
		//printf("Error: TickType:%d\n", field);
...
void TestCppClient::tickString(TickerId tickerId, TickType tickType, const std::string& value) {
	...
	//switch (tickType) {
	//case LAST_TIMESTAMP:
	//	strcpy(m_depthList[index].time, value.c_str());
	//	break;
	//}
...
void TestCppClient::contractDetails( int reqId, const ContractDetails& contractDetails) {
	...
	strcpy(m_contractInfo[m_numOfContractsInfo].symbol, contractDetails.summary.symbol.c_str());
	m_contractInfo[m_numOfContractsInfo].conId = contractDetails.summary.conId;
	strcpy(m_contractInfo[m_numOfContractsInfo].multiple, contractDetails.summary.multiplier.c_str());
	strcpy(m_contractInfo[m_numOfContractsInfo].timezone, contractDetails.timeZoneId.c_str());
	strcpy(m_contractInfo[m_numOfContractsInfo].trading, contractDetails.tradingHours.c_str());
	m_numOfContractsInfo++;
}
//! [contractdetails]
...
void TestCppClient::execDetails( int reqId, const Contract& contract, const Execution& execution) {
	...
	if (execution.orderId == m_response.reqId)
	{
		if (m_semaphore != NULL && m_response.doc != NULL)
		{
			Value dateStr;
			dateStr.SetString(contract.lastTradeDateOrContractMonth.c_str(), contract.lastTradeDateOrContractMonth.length(), m_response.doc->GetAllocator());
			m_response.doc->AddMember(rapidjson::StringRef("expire"), dateStr, m_response.doc->GetAllocator());

			Value symbolStr;
			symbolStr.SetString(contract.symbol.c_str(), contract.symbol.length(), m_response.doc->GetAllocator());
			m_response.doc->AddMember(rapidjson::StringRef("symbol"), symbolStr, m_response.doc->GetAllocator());

			m_response.doc->AddMember(rapidjson::StringRef("shares"), execution.shares, m_response.doc->GetAllocator());
			m_response.doc->AddMember(rapidjson::StringRef("price"), execution.avgPrice, m_response.doc->GetAllocator());
			m_response.doc->AddMember(rapidjson::StringRef("amount"), execution.cumQty, m_response.doc->GetAllocator());
			m_response.doc->AddMember(rapidjson::StringRef("status"), rapidjson::StringRef("success"), m_response.doc->GetAllocator());
...
void TestCppClient::historicalData(TickerId reqId, const std::string& date, double open, double high,
	...
	if (m_klines != NULL) {
		if (date.find("finish") == std::string::npos) {
			m_klines[m_klinesCounter].open = open;
			m_klines[m_klinesCounter].high = high;
			m_klines[m_klinesCounter].low = low;
			m_klines[m_klinesCounter].close = close;
			m_klines[m_klinesCounter].volume = volume;
			strcpy(m_klines[m_klinesCounter].date, date.c_str());
			m_klinesCounter++;
		}
		else {
			if (m_semaphore != NULL) 
...
void TestCppClient::position( const std::string& account, const Contract& contract, double position, double avgCost) {
	...
	if (m_response.reqId == 8888)
	{
		Value positionStr;
		positionStr.SetString(contract.symbol.c_str(), contract.symbol.length(), m_response.doc->GetAllocator());
		m_response.doc->AddMember(rapidjson::StringRef("symbol"), positionStr, m_response.doc->GetAllocator());

		Value expireStr;
		expireStr.SetString(contract.lastTradeDateOrContractMonth.c_str(), contract.lastTradeDateOrContractMonth.length(), m_response.doc->GetAllocator());
		m_response.doc->AddMember(rapidjson::StringRef("expire"), expireStr, m_response.doc->GetAllocator());

		Value mulStr;
		mulStr.SetString(contract.multiplier.c_str(), contract.multiplier.length(), m_response.doc->GetAllocator());
		m_response.doc->AddMember(rapidjson::StringRef("multiple"), mulStr, m_response.doc->GetAllocator());


		m_response.doc->AddMember(rapidjson::StringRef("position"), position, m_response.doc->GetAllocator());
		m_response.doc->AddMember(rapidjson::StringRef("avgCost"), avgCost, m_response.doc->GetAllocator());
	}
}
//! [position]
...
void TestCppClient::positionEnd() {
	printf( "PositionEnd\n");
	if (m_response.reqId == 8888&&m_semaphore!=NULL) 
	{
		ReleaseSemaphore(m_semaphore, 1, NULL);
	}
}
...
void TestCppClient::accountSummary( int reqId, const std::string& account, const std::string& tag, const std::string& value, const std::string& currency) {
	...
	if (reqId == m_response.reqId) {
		if (m_response.doc != NULL)
		{
			//Value accountStr;
			//accountStr.SetString(account.c_str(), account.length(), m_response.doc->GetAllocator());
			Value tagStr;
			tagStr.SetString(tag.c_str(), tag.length(), m_response.doc->GetAllocator());
			Value valueStr;
			valueStr.SetString(value.c_str(), value.length(), m_response.doc->GetAllocator());
			//m_response.doc->AddMember(rapidjson::StringRef("account"), accountStr, m_response.doc->GetAllocator());
			m_response.doc->AddMember(tagStr, valueStr, m_response.doc->GetAllocator());
		}
	}
}
//! [accountsummary]
